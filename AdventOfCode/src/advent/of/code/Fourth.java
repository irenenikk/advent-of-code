...
public class Fourth implements Solution{
    ...
    private static void oneStar(String input) {
    ...
    private static  void twoStars(String input) {
        String[] phrases = input.split("\n");
        int validPhrases = 0;
        for (String p : phrases) {
            String[] words = p.split(" ");
            Set<Set<Character>>[] letters = new HashSet[9999];
            for (int i = 0; i < letters.length; i++) {
                letters[i] = new HashSet();
                letters[i].add(new HashSet<>());
            }
            ...
            for (String word: words) {
                ...
                Set<Set<Character>> setsWithSameAmountOfLetters = letters[characters.size()];
                for (Set<Character> set: setsWithSameAmountOfLetters){
                ...
                letters[characters.size()].add(characters);
            }
            if (allWordsAreValid) {
                validPhrases++;
            }
