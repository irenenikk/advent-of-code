 ...
 */
public class Seventh implements Solution {
    private Map<Disc, Set<Disc>> discChildren;
    private Map<String, Disc> discsByName;
    ...
    private Disc oneStar() {
        inputLines = new ArrayList<>();
        Set<String> heldPrograms = new HashSet<>();
        Set<String> holdingPrograms = new HashSet<>();
        discsByName = new HashMap<>();
        ...
        while (reader.hasNextLine()) {
            ...
            String[] programInfo = holder[0].split(" ");
            String programName = programInfo[0].trim();
            inputLines.add(input);
            int weight = getProgramWeight(programInfo);
            Disc disc = new Disc(programName, weight);
            discsByName.put(programName, disc);
            if (holder.length > 1) {
                String[] programs = holder[1].split(",");
                Arrays.stream(programs).map(p -> p.trim()).forEach(p -> heldPrograms.add(p));
                holdingPrograms.add(programName);
            }
        }
        for (Disc p: discsByName.values()) {
            if (holdingPrograms.contains(p.name) && !heldPrograms.contains(p.name)) {
                return p;
            }
        }
        return null;
    ...
    private void twoStars(Disc rootDisc) {
        this.discChildren = new HashMap<>();
        constructTree();
        calculateChildrenWeights(rootDisc);
    }

    private long calculateChildrenWeights(Disc disc) {
        Set<Disc> discsOnTop = this.discChildren.get(disc);
        if (discsOnTop == null) {
            return disc.weight;
        }
        long childrenWeights = 0;
        Map<Long, Disc> lastDiscsByWeight = new HashMap<>();
        Map<Long, Integer> weightFrequencies = new HashMap<>();
        for (Disc d : discsOnTop) {
            long childrenWeightForD = calculateChildrenWeights(d);
            int frequency = 0;
            if (weightFrequencies.containsKey(childrenWeightForD)) {
                frequency = weightFrequencies.get(childrenWeightForD);
            }
            int newFrequency = frequency + 1;
            weightFrequencies.put(childrenWeightForD, newFrequency);
            childrenWeights += childrenWeightForD;
            lastDiscsByWeight.put(childrenWeightForD, d);
        }
        WeightInfo unbalancedWeightInfo = findGoalWeightUnbalancedProgram(weightFrequencies);
        if (unbalancedWeightInfo != null) {
            Disc lastDisc = lastDiscsByWeight.get(unbalancedWeightInfo.unBalancedWeigth);
            long differenceToIdeal = unbalancedWeightInfo.goalWeight - unbalancedWeightInfo.unBalancedWeigth;
            System.out.println(lastDisc.weight + differenceToIdeal);
        }
        return disc.weight + childrenWeights;
    }
    
    private WeightInfo findGoalWeightUnbalancedProgram(Map<Long, Integer> weightFrequencies) {
        long unBalanced = -1;
        long goal = -1;
        for (Map.Entry<Long, Integer> e : weightFrequencies.entrySet()) {
            long weight = e.getKey();
            int frequency = e.getValue();
            // only one program is unbalanced
            if (frequency == 1 && weightFrequencies.size() > 1) {
                unBalanced = weight;
            } else {
                goal = weight;
            }
        ...
        if (unBalanced > 0) {
            return new WeightInfo(unBalanced, goal);
        }
        return null;
    }
    ...
    private void constructTree() {
        inputLines.stream().forEach(l -> {
            String[] holder = l.split("->");
            String[] programInfo = holder[0].split(" ");
            String programName = programInfo[0];
            if (holder.length > 1) {
                String[] programs = holder[1].split(",");
                Set<Disc> heldPrograms = new HashSet<>();
                Arrays.stream(programs).map(p -> p.trim()).forEach(p -> heldPrograms.add(discsByName.get(p)));
                discChildren.put(discsByName.get(programName), heldPrograms);
            }
        });
    }

    private int getProgramWeight(String[] programInfo) {
        String weightInfo = programInfo[1];
        int i = weightInfo.indexOf("(") + 1;
        String integer = "0";
        while (weightInfo.charAt(i) != ')') {
            integer += weightInfo.charAt(i);
            i++;
        }
    ...
    private class Disc {
        ...
        public boolean equals(Object o) {
            ...
            if (weight != disc.weight) return false;
            return name != null ? name.equals(disc.name) : disc.name == null;
        }
    ...
    private class WeightInfo {
        long unBalancedWeigth;
        long goalWeight;

        public WeightInfo(long unBalancedWeigth, long goalWeight) {
            this.unBalancedWeigth = unBalancedWeigth;
